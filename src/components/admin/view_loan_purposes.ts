...
export default {
    ...
    oncreate(_vnode: Vnode) {
        ...
        $(document).ready(function() {
            $("#datatable").DataTable({
                ajax: {
                    url: AppSettings.API_BASE_URL + "/api/loan-purpose/list",
                    type: "GET",
                    ...
                    dataSrc: function(json: any) {
                        m.redraw();
                        json.loanPurposes.map(function(v) {
                            v._id = v._id.toUpperCase();
                            v.uid = v._id.slice(-6);
                            v.status = statusConvert(v.status);
                        });
                        return json.loanPurposes;
                    }
                },
                dom: "Bfrtip",
                buttons: [
                    {
                        text: "New Loan Purpose",
                        action: function(e: any, dt: any, node: any, config: any) {
                            m.route.set("/admin/new-loan-purpose");
                        }
                    },
                ],
                columns: [
                            ...
                            m(".page-title-box", [
                                m(".btn-group.pull-right",
                                    m("ol.breadcrumb.hide-phone.p-0.m-0", [
                                        ...
                                        m("li.breadcrumb-item.active", "Loan Purposes")
                                    ])
                                ),
                                m("h4.page-title", "Loan Purposes")
                            ])
                        )
                    ),
                    m(".row",
                        m(".col-12",
                            m(".card-box.table-responsive", [
                                m("h4.m-t-0.header-title", "Loan Purposes"),
